...
Reviewing the README structure and its contract sections.
Reviewing the README structure and its contract sections.
Your Game and Exam contract is missing from this file. This is the README on development, and your contract (sections 5.1–5.3) is still waiting in the Lab 0 PR feature/game-exam-contract.

So merge the Lab 0 PR first, then edit the README on a new branch (docs/lab1-game-exam, from step 4 of my previous message). If you edit this version, your changes will conflict with your own contract PR.

After the Lab 0 merge, change the six places below. Each part says where it goes.

1. Team table, your row
Fill in your GitHub handle. It's the account you push with; from our earlier chat that looks like AlexanderMcMarshall.

markdown
| Alexandru Bujor | alexandru.bujor@isa.utm.md | @AlexanderMcMarshall | Game Service + Exam Service |
2. Section 4 (Technologies), your two rows
markdown
...
3. Section 5.2 (Game Service): add at the end of the section
This also replaces the existing "Calls Game Service makes to other services" table with the exact shapes.

markdown
#### Lab 1 updates
- **Session** also returns `"startedAt": "ISO | null"`, the time the first player joined, which is when the day/night cycle starts.
...
**Calls Game Service makes** (mocked in Lab 1, to confirm with each owner):
...
4. Section 5.3 (Exam Service): add at the end of the section
markdown
#### Lab 1 updates
...
5. Section 7 (Repository Links), your two rows
markdown
...
6. Section 8 (Postman Collections), and a new section 10
In section 8, add under the existing line:
...
Your teammates add their own rows to sections 4, 7 and 10 in the same format.

Don't change sections 1–3, 6 and 9, or the "To complete" note in section 4. Those are team-wide.

When you're done, commit on the docs/lab1-game-exam branch:

bash
git add README.md &&
git commit -m "docs: update game and exam contract, technologies and deployment info" &&
git push -u origin docs/lab1-game-exam
full mkdown changed file

Created a file, shared files
Compiling a full README using earlier contract details.
Presented file
The complete README is ready to download below. It's your current development README with the following added:

your Game and Exam contract from the Lab 0 PR (sections 5.1–5.3);
the Lab 1 updates in 5.2 and 5.3;
your rows in sections 4 and 7, your GitHub handle, and your Postman files in section 8;
a new section 10 (Running the Services) and section 11 (Changelog).
Everything else is unchanged.

Check before using it:

Your GitHub handle: I put @AlexanderMcMarshall in the team table. Change it if that's not your account.
Mihai's changes: if Mihai merges his contract PR (docs/world-zombie-contract) before you, his section 5.4 will be on development and this file would overwrite it. In that case, copy his section into this file before committing. Run git diff after replacing the file: every removed line (red, starting with -) should be something you meant to replace.
How to use it (after the Lab 0 PR is merged):

Download README.md into Downloads.
Run:
bash
cd /c/dev/IV-year/PAD/main &&
git checkout development && git pull &&
git checkout -b docs/lab1-game-exam &&
cp ~/Downloads/README.md README.md &&
git diff --stat
git diff --stat should show roughly README.md | ~70 +++--: mostly additions, and very few removals. If you see hundreds of deletions, stop and send me the output.
3. Commit it together with the submodule update:

bash
git submodule update --init &&
git submodule update --remote services/game-service services/exam-service &&
git add README.md services &&
git commit -m "docs: update game and exam contract, technologies and deployment info" &&
git push -u origin docs/lab1-game-exam
Open the PR docs/lab1-game-exam → development, and Squash and merge it after the approval.
